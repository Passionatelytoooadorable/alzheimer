...
class Journal {
    ...
    loadSampleData() {
        ...
        // Sample journal entries - only 2 entries this week
        const sampleEntries = [
            {
                id: 1,
    ...
    setupEventListeners() {
        ...
        // Listen for storage updates from dashboard
    ...
    addResetButton() {
        ...
        resetBtn.addEventListener('click', function() {
            ...
            localStorage.removeItem('journalCount');
            localStorage.removeItem('weeklyCount');
            location.reload();
        });
    ...
    showEntriesForDate(date) {
        ...
        if (dateEntries.length > 0) {
            ...
            this.displayEntries('all'); // Reset to show all, then we'll manually filter
    ...
    handleJournalSubmit(e) {
        ...
        // Update dashboard stats - SYNC ADDED HERE
        this.updateDashboardStats();
    ...
    updateDashboardStats() {
        ...
        console.log('Dashboard stats updated - Journal count:', journalCount, 'Weekly count:', weeklyCount, 'Reminder count:', todayReminders.length);
    ...
    // Add this method to calculate weekly count
    calculateWeeklyCount() {
        const entries = JSON.parse(localStorage.getItem('journalEntries')) || [];
        const oneWeekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
        const weeklyEntries = entries.filter(entry => entry.date >= oneWeekAgo);
    ...
    handleReminderSubmit(e) {
        ...
        const reminder = {
            ...
            completed: false,
            type: 'normal' // Add type for AI companion compatibility
        };
