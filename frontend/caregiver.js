...
// ============================================================
// GEOFENCE BREACH (#7) — patient POSTs when outside safe zones
// Backend logs it and surfaces it to the linked caregiver
// ============================================================
// Add this to routes/locations.js (the /api/locations router) OR keep here.
// We keep it in caregiver.js so it has access to the caregiver link logic.
//
// Frontend calls: POST /api/locations/geofence-breach
// Register in server.js: app.use('/api/locations', locationRoutes)
// OR add a dedicated route here and register as /api/caregiver/geofence-breach
//
// We expose it as POST /api/caregiver/geofence-breach so it works with the
// existing caregiver router mount in server.js.

router.post('/geofence-breach', auth, requireRole('patient'), async (req, res) => {
  try {
    const { latitude, longitude, nearest_zone, breached_at } = req.body;
    const patientId = req.user.userId;

    // Log as a user activity so caregiver feed picks it up
    await query(
      ...
      [
        patientId,
        'geofence_breach',
        'Patient left all safe zones — nearest: ' + (nearest_zone || 'unknown') +
        '. Location: ' + (latitude ? latitude.toFixed(5) : '?') + ', ' + (longitude ? longitude.toFixed(5) : '?')
      ]
    );

    // Also save the location so caregiver map updates
    if (latitude && longitude) {
      await query(
        `INSERT INTO user_locations (user_id, latitude, longitude, address)
         VALUES ($1, $2, $3, $4)`,
        [patientId, latitude, longitude, 'GEOFENCE BREACH — outside safe zone']
      ).catch(() => {}); // non-fatal if locations table differs
    }

    // Optionally: send a caregiver_message automatically
    const link = await query(
      `SELECT caregiver_id FROM caregiver_patient_links
       WHERE patient_id = $1 AND status = 'accepted'`,
      [patientId]
    );

    if (link.rows.length) {
      const caregiverId = link.rows[0].caregiver_id;
      const patient = await query(
        'SELECT name FROM users WHERE id = $1',
        [patientId]
      );
      const patientName = (patient.rows[0] && patient.rows[0].name) || 'Your patient';
      ...
      await query(
        ...
        [
          caregiverId,
          patientId,
          '🚨 GEOFENCE ALERT: ' + patientName + ' has left all safe zones. ' +
          'Nearest safe zone: ' + (nearest_zone || 'unknown') + '. ' +
          'Breached at: ' + (breached_at || new Date().toISOString()) + '. ' +
          'Please check their location on your caregiver dashboard.'
        ]
      ).catch(() => {}); // non-fatal
    }

    res.json({ message: 'Breach logged successfully' });
  } catch (err) {
    if (process.env.NODE_ENV !== 'production') console.error('geofence-breach error:', err);
    res.status(500).json({ error: pgErrMsg(err) });
  }
});
